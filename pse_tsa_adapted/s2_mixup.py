...
#------------------- #  mixup block --> note: x contains (data, mask)-------------------
        ...
        outputs = model((inputs, mask), dates) #model input =tuple of data and mask
        loss = mixup_criterion(criterion, outputs, targets_a, targets_b, lam)
        loss_meter.add(loss.item())
        
        pred = outputs.detach()
        y_p = pred.argmax(dim=1)
        y_pred.extend(list(y_p.cpu().numpy()))
        
        total += y.size(0)
        correct += (lam * y_p.eq(targets_a.data).cpu().sum().float()
            + (1 - lam) * y_p.eq(targets_b.data).cpu().sum().float())
...
def evaluation(model, criterion, loader, device, config, mode='val'):
    y_true = []
    y_pred = []
    probs = [] #collect class probabilities
    ...
    for (x, y, dates) in loader: #call dates for tsa
        ...
        y_pred.extend(list(y_p))
        probs.extend(list(prediction.cpu().numpy())) #add class probabilities to list
    ...
    if mode == 'val':
        ...
        return metrics, confusion_matrix(y_true, y_pred, labels=list(range(config['num_classes']))), y_true, y_pred, probs #return extended to include y_true and y_pred
...
def save_results(metrics, conf_mat, config, y_true, y_pred, probs): #extend save results to include y_true, y_pred and class probabilities
...
def main(config):
    ...
    for _, (train_loader, val_loader, test_loader) in enumerate(loaders):
        ...
        test_metrics, conf_mat, y_true, y_pred, probs = evaluation(model, criterion, test_loader, device=device, mode='test', config=config) #add ytrue and ypred to save. add probs

        print('Loss {:.4f},  Acc {:.2f},  IoU {:.4f}'.format(test_metrics['test_loss'], test_metrics['test_accuracy'],
                                                             test_metrics['test_IoU']))
                                                             
        save_results(test_metrics, conf_mat, config, y_true, y_pred,probs) #save extended to include y_true, y_pred and class probabilities
