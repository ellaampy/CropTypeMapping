...
def save_results(metrics, conf_mat, config, y_true, y_pred):
...
def main(config):
    ...
    for _, (train_loader, val_loader, test_loader) in enumerate(loaders):
        ...
        model_config = dict(input_dim=config['input_dim'], mlp1=config['mlp1'], pooling=config['pooling'],
                            ...
                            positions=None, fusion_type = config['fusion_type'],
                            mlp4=config['mlp4'])
        ...
        test_metrics, conf_mat, y_true, y_pred = evaluation(model, criterion, test_loader, device=device, mode='test', config=config) 

        print('Loss {:.4f},  Acc {:.2f},  IoU {:.4f}'.format(test_metrics['test_loss'], test_metrics['test_accuracy'],
                                                             test_metrics['test_IoU']))
                                                             
        save_results(test_metrics, conf_mat, config, y_true, y_pred) 
